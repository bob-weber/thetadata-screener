...
```bash
# Screener app (starts the ThetaData terminal automatically)
gui-env/bin/python run_screener.py

# Portfolio tracker (no ThetaData terminal required)
...
- **`screener.py`** — all screener logic; the screener GUI calls this exclusively.
...
Built with PyQt6. `run_screener.py` invokes `start_terminal.sh` before creating the `QApplication`, and hooks `stop_terminal.sh` to `aboutToQuit`.
...
`run_portfolio.py` launches a standalone window with no ThetaData dependency.
...
The Java terminal (`ThetaTerminalv3.jar`) must be running before any screener data fetch. It listens on port `25503` (v3 REST) and authenticates using credentials in `creds.txt` (gitignored). `config.toml` controls terminal settings (host, ports, FPSS streaming).
